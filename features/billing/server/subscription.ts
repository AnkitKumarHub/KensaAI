...
export async function getUserSubscription(
  ...
  if (!user) {
    return { plan: Plan.FREE, status: "active", renewsAt: null };
  }

  const renewsAt = user.subscriptionRenewsAt?.toISOString() ?? null;

  if (user.plan !== Plan.PRO) {
    return { plan: Plan.FREE, status: "active", renewsAt };
  }

  if (user.subscriptionStatus === "pending") {
    return { plan: Plan.FREE, status: "trialing", renewsAt };
  }
  ...
  if (user.subscriptionStatus === "canceled") {
    ...
    if (stillActive) {
      return { plan: Plan.PRO, status: "active", renewsAt };
    }

    return { plan: Plan.FREE, status: "canceled", renewsAt };
  }
...
export async function createProSubscription(userId: string) {
  ...
  if (subscription.plan === Plan.PRO && subscription.status === "active") {
    throw new Error("You already have an active Pro subscription.");
  }

  const planId = process.env.RAZORPAY_PLAN_ID_INDIVIDUAL;
  if (!planId) {
